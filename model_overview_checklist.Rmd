---
title: "Model overview checklist"
...
This document exists in the repository at [https://github.com/codatmo/Simple_SIR](https://github.com/codatmo/Simple_SIR) as `model_overview.Rmd` and rendered in html as `model_overview.html`.
...
```{r echo=TRUE, message=FALSE, comment=NA}
niter <- 2000
model <- stan_model("stan/sir_negbin.stan")
...
```{r comment=NA}
stan_diag(fit_sir_negbin,information="divergence")
```
...
fit.df <- as.data.frame(summary(
  fit_sir_negbin, pars = "pred_cases", probs = c(0.05, 0.5, 0.95))$summary)
...
The prior predictive check estimates the model parameters without data being used. The resulting draws are then used to provide a human interpretable application of parameter values that reflect modeling goals. In this case we predict new data via predictive application of the likelihood without having seen any data. Note that `compute_likelihood = 0` prevents the likelihood combining data information with the priors. 

```{r echo=TRUE, comment=NA}
compute_likelihood = 0 # will not compute likelihood, allows for prior predictive check
# data for Stan
data_sir <- list(n_days = n_days, y0 = y0, t0 = t0, ts = ts, N = N, cases = cases, compute_likelihood = compute_likelihood)

model <- stan_model("stan/sir_negbin.stan")
fit_sir_negbin_prior_pred_chck <- sampling(model,
                data = data_sir,
                iter = niter,
                chains = 4,
                seed=1590291619) #this seed works, not all will

pars=c('beta', 'gamma', "R0", "recovery_time")
print(fit_sir_negbin_prior_pred_chck, pars = pars)
...
s_prior <- rstan::extract(fit_sir_negbin_prior_pred_chck)
